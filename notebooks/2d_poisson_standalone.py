...
class SpectralInterpolationND(nn.Module):
    def __init__(self, Ns, bases, domains):
        ...
        # Set up nodes and frequencies (as needed)
    ...
    def _fourier_interpolate_1d(self, x_eval, values, to_std, k):
        ...
        N = values.shape[-1]
        coeffs = torch.fft.fft(values, dim=-1)
        x_eval_standard = to_std(x_eval)

        x_matrix = x_eval_standard.unsqueeze(1) * k.unsqueeze(0)
        fourier_matrix = torch.exp(1j * x_matrix)
        result = torch.real(fourier_matrix @ coeffs.T).T / N
    ...
    def _compute_cheb_diff_matrix(self, n, domain_length):
        """Compute Chebyshev differentiation matrix"""
        nodes = torch.cos(torch.pi * torch.linspace(0, 1, n))
        ...
        # Scale for domain transformation
        return D * (2.0/domain_length)
        
    def _compute_fourier_diff_matrix(self, n, domain_length):
        """Compute Fourier differentiation matrix"""
        D = torch.zeros((n, n), dtype=torch.float64)
        
        i, j = torch.meshgrid(torch.arange(n, dtype=torch.float64), 
        ...
        # Scale for domain transformation
        return D * (2*np.pi/domain_length)
            
    def derivative_matrix_x(self, k=1):
        """Get k-th derivative matrix in x direction"""
        if k == 0:
            nx = len(self.nodes_x)
            return torch.eye(nx, dtype=torch.float64)
            
        if k not in self._diff_matrices_x:
            if 1 not in self._diff_matrices_x:
                if self.basis_x == 'fourier':
                    self._diff_matrices_x[1] = self._compute_fourier_diff_matrix(
                        len(self.nodes_x), self.domain_x_length)
                else:
                    self._diff_matrices_x[1] = self._compute_cheb_diff_matrix(
                        len(self.nodes_x), self.domain_x_length)
            
            # Compute k-th derivative matrix by composition
            Dk = self._diff_matrices_x[1]
            for i in range(k-1):
                Dk = Dk @ self._diff_matrices_x[1]
            self._diff_matrices_x[k] = Dk
            
        return self._diff_matrices_x[k]
        
    def derivative_matrix_y(self, k=1):
        """Get k-th derivative matrix in y direction"""
        if k == 0:
            ny = len(self.nodes_y)
            return torch.eye(ny, dtype=torch.float64)
            
        if k not in self._diff_matrices_y:
            if 1 not in self._diff_matrices_y:
                if self.basis_y == 'fourier':
                    self._diff_matrices_y[1] = self._compute_fourier_diff_matrix(
                        len(self.nodes_y), self.domain_y_length)
                else:
                    self._diff_matrices_y[1] = self._compute_cheb_diff_matrix(
                        len(self.nodes_y), self.domain_y_length)
            
            # Compute k-th derivative matrix by composition
            Dk = self._diff_matrices_y[1]
            for i in range(k-1):
                Dk = Dk @ self._diff_matrices_y[1]
            self._diff_matrices_y[k] = Dk
            
        return self._diff_matrices_y[k]
